...
import { User, Shield, Check, ChevronRight, LogOut, Settings, HelpCircle, Globe, Bell, X, Moon, Sun } from 'lucide-react';
    ...
    <div className={`pb-32 min-h-screen ${theme === 'dark' ? 'bg-[#0F172A]' : 'bg-[#F8FAFC]'}`}>
      ...
      <div className={`p-8 pt-16 flex flex-col items-center space-y-4 border-b ${theme === 'dark' ? 'bg-slate-900 border-white/5' : 'bg-white border-slate-100'}`}>
        <div className="relative">
          <div className="w-24 h-24 bg-blue-600 rounded-[2.5rem] flex items-center justify-center text-white text-3xl font-black shadow-xl shadow-blue-200 dark:shadow-none">
            AR
          </div>
        ...
        <div className="text-center">
          <h2 className={`text-xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>Arjun Reddy</h2>
          <p className="text-sm text-slate-400 font-medium">Verified Farmer • East Midnapore</p>
        </div>
      </div>
